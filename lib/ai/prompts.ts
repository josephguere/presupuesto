...
export function buildIntentSystemInstruction(catalog: Catalog, now: Date = new Date()): string {
  ...
  return [
    ...
    "  monthly_evolution     cómo ha ido el gasto en el tiempo, por mes o por",
    "                        día: «evolución», «tendencia», «últimos N meses»,",
    "                        «gasto día a día», «por días», «cómo viene»",
    "",
    ...
    "  · comentario: rellénalo SOLO si el usuario habla del comentario, la nota",
    "    o la descripción del movimiento («que digan Hanna», «con Lley en el",
    "    comentario», «relacionados con X»). Copia solo la palabra a buscar, no",
    "    la frase entera. La búsqueda es parcial y no distingue mayúsculas.",
    "    Si menciona un COMERCIO, usa comercio; el comentario es otra cosa.",
    "  · limite solo si pidió una cantidad concreta («mis cinco movimientos más altos»).",
    "  · unidadEvolucion y cantidadEvolucion solo con monthly_evolution. Pon",
    "    unidadEvolucion=\"dia\" si pidió ver por días («día a día», «diario»,",
    "    «por días»); en cualquier otro caso, incluida la evolución sin más",
    "    detalle, usa \"mes\". cantidadEvolucion es cuántas unidades, si lo dijo",
    "    («últimos 15 días», «últimos 3 meses»). Esa intención mira su propia",
    "    ventana de tiempo: NO le pongas período.",
    "",
