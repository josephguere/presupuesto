...
/**
 * La ingesta de correos, de punta a punta.
 *
 * Vive en `lib/` y no dentro de un Route Handler porque la sirven DOS rutas
 * —`/api/ingest/email`, que es la buena, y `/api/ingest/bcp`, que se conserva
 * para no romper los Apps Script ya configurados—. Tenerla aquí es lo que hace
 * que sean literalmente la misma lógica y no dos copias que un día divergen.
 *
 *     Correo  →  autorizar  →  identificar proveedor  →  parser del proveedor
 *                                  ↓
 *             Supabase  ←  insertar  ←  convertir divisa  ←  normalizar
 *
 * LO ÚNICO QUE CAMBIA POR PROVEEDOR ES EL PARSER. Guardar el correo crudo,
 * deduplicar, convertir a soles, insertar y registrar es común y no tiene una
 * sola rama por entidad: `lib/parsers/providers.ts` decide quién lee el correo
 * y aquí se trata el resultado igual venga de donde venga.
 *
...
/** Tope defensivo: un correo de notificación no llega ni de lejos a este tamaño. */
...
export async function handleIngestPost(request: Request): Promise<NextResponse> {
  // ---- 1. Autorización -----------------------------------------------------
  ...
  // El filtro de asunto es del BCP —su variable lleva su nombre— y no se aplica
  // a los demás proveedores: si lo hiciera, configurarlo con una frase del banco
  // dejaría fuera todos los correos de Yape sin ningún aviso.
  const senderProvider = providerForSender(payload.from);
  if (senderProvider?.id === "BCP" && !matchesSubjectFilter(payload.subject)) {
  ...
  try {
    return await ingest(payload, senderProvider);
  } catch (error) {
 ...
 */
export async function handleIngestGet(
  request: Request,
  endpoint: string,
  ...
  return NextResponse.json({
    ...
    // Para poder comprobar desde Apps Script que este despliegue ya entiende
    // Yape, sin tener que mandar un correo de prueba.
    providers: ["BCP", "YAPE"],
  });
}
...
async function ingest(
  ...
  senderProvider: EmailProvider | null,
): Promise<NextResponse> {
  // ---- Idempotencia: ¿ya conocemos este correo? ---------------------------
  //
  // Es la MISMA deduplicación para todos los proveedores, y no hace falta nada
  // específico de Yape: `gmail_message_id` es único en la tabla, así que un
  // yapeo no puede entrar dos veces aunque Apps Script relea el correo. El
  // número de operación de Yape queda como dato del movimiento, no como clave.
  ...
  // ---- ¿Quién sabe leer este correo? --------------------------------------
  const provider = identifyProvider({ from: payload.from, rawBody: payload.rawBody });
  ...
  //
  // El correo se guarda AUNQUE no haya parser que lo entienda: es evidencia, y
  // es lo que permite corregir el parser mañana y reprocesarlo.
  const email = await storeEmail(payload, provider ?? senderProvider);

  logger.info("ingest.email_stored", {
    gmailMessageId: payload.gmailMessageId,
    emailId: email.id,
    provider: provider?.id ?? "DESCONOCIDO",
    reused: Boolean(existing),
    rawBodyLength: payload.rawBody.length,
  });

  // Remitente conocido pero contenido que no es una operación: publicidad, un
  // extracto, un aviso de seguridad. Se marca y se deja estar; registrarlo como
  // movimiento sería inventar un gasto.
  if (!provider) {
    return await failParse(email.id, payload, {
      code: "NOT_A_MATCH",
      message:
        "El correo viene de un remitente conocido pero su contenido no es una " +
        "notificación de operación que sepamos interpretar.",
      missingFields: [],
    });
  }

  // ---- Parseo -------------------------------------------------------------
  const parseResult = provider.parse(payload.rawBody);
...
/**
 * Deja constancia de que el correo no se pudo interpretar.
 *
 * Responde 200 A PROPÓSITO: el correo está guardado y reintentar no cambiaría
 * nada. Si respondiera 5xx, Apps Script lo reenviaría en cada disparo del
 * trigger para siempre.
 */
async function failParse(
  ...
  await updateEmailStatus(emailId, "PARSE_ERROR", error.message);
...
async function upsertTransaction(
  ...
  const { data, error } = await getSupabaseAdmin()
    ...
    .insert({
      ...
      origin: "EMAIL",
      // Todo movimiento nace contabilizado, venga de donde venga. Quien decida
      // lo contrario es el usuario, desde el formulario.
      contabilizar: true,
 ...
 *
 * Yape solo opera en soles, asi que sus correos no llegan nunca hasta aqui.
 */
